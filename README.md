...
La PWA y la API deben estar en el mismo sitio HTTPS o detrás de un proxy de origen compatible con cookies. La vinculación comprueba que el navegador conserve la cookie antes de consumir la clave; si falla, muestra el error y permite reintentar con la misma clave tras corregir el despliegue. El equipo debe permanecer físicamente bajo control de la caseta. Las lecturas se auditan por equipo, no por empleado. Entrada y salida se alternan por pase. La pantalla tiene dos secciones: **Escanear** (escáner y resultado) y **Bitácora** (quién sigue dentro y las lecturas de las últimas 12 horas del equipo; lado a lado en pantallas anchas y en pestañas en teléfono). La pestaña Bitácora muestra cuántas visitas hay dentro y el equipo recuerda la última sección abierta. Cada resultado se confirma con vibración y un tono (uno agudo si se acepta, dos graves si se rechaza). Antes de publicar este cambio, despliega la migración y el backend nuevos; una PWA nueva frente a la API anterior no podrá vincular el equipo.
