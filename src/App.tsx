...
function Shell({ path, navigate }: { path: string; navigate: (p: string) => void }) {
  ...
  const role = identity!.user.globalRole;
  const alerts = useQuery<Page<Notification>>(role !== 'ADMIN' && property ? `/auth/devices/notifications?propertyId=${property.id}` : null, 30000);
  const nav = role === 'ADMIN'
    ? [{ path: '/admin/invitaciones', label: 'Administración', Icon: AnimatedSlidersHorizontal }]
    : role === 'GUARD'
      ? [{ path: '/caseta', label: 'Caseta', Icon: ScanLine }, { path: '/dispositivos', label: 'Dispositivos', Icon: Smartphone }]
      : [{ path: '/', label: 'Inicio', Icon: AnimatedHome }, { path: '/pases', label: 'Mis pases', Icon: AnimatedQr }, { path: '/dispositivos', label: 'Dispositivos', Icon: Smartphone }];
  function go(p: string) { setMobile(false); navigate(role === 'ADMIN' ? (adminSections.some(section => section.path === p) ? p : '/admin/invitaciones') : role === 'GUARD' && (p === '/' || p === '/pases') ? '/caseta' : p); requestAnimationFrame(() => document.getElementById('main')?.focus()); }
  return <div className={`app-layout${role === 'ADMIN' ? ' admin-layout' : ''}`}><a href="#main" className="skip-link">Ir al contenido</a>{mobile && <button className="sidebar-scrim" aria-label="Cerrar menú" onClick={() => setMobile(false)}/>}<aside ref={sidebar} id="main-navigation" role={mobile ? 'dialog' : undefined} aria-modal={mobile || undefined} aria-label="Menú de Zentry" className={`sidebar ${mobile ? 'open' : ''}`}><div className="sidebar-brand"><Brand onHome={() => go('/')}/><AnimatedX className="mobile-close" label="Cerrar menú" onClose={() => setMobile(false)}/></div><div className="community-label"><span className="connection-dot"/> Tu comunidad</div><nav aria-label="Navegación principal">{nav.map(({ path: p, label, Icon }) => <button key={p} className={path === p || (role === 'ADMIN' && path.startsWith('/admin/')) ? 'active' : ''} aria-current={path === p || (role === 'ADMIN' && path.startsWith('/admin/')) ? 'page' : undefined} onClick={() => { if (p === '/admin/invitaciones' && mobile) afterSlidersAnimation(() => go(p)); else go(p); }}><Icon size={20}/>{label}{(path === p || (role === 'ADMIN' && path.startsWith('/admin/'))) && <span className="nav-dot"/>}</button>)}</nav>{role !== 'ADMIN' && <div className="sidebar-install"><div className="install-art"><Smartphone size={29}/><span><ShieldCheck size={15}/></span></div><h3>Zentry en tu teléfono</h3><p>Instala Zentry en tu teléfono y abre la app con un toque.</p><InstallButton/></div>}<div className="sidebar-bottom">{role !== 'ADMIN' && <button onClick={() => go('/ayuda')} className={path === '/ayuda' ? 'active' : ''}><CircleHelp size={19}/> Ayuda y primeros pasos</button>}<button onClick={() => void logout().catch(() => {})}><LogOut size={19}/> Cerrar sesión</button><div className="sidebar-user"><span className="avatar">{identity!.user.fullName.slice(0, 1)}</span><span><strong>{identity!.user.fullName}</strong><small>{role === 'ADMIN' ? 'Administración' : role === 'GUARD' ? 'Personal de caseta' : 'Residente'}</small></span></div></div></aside>
    <div className="app-main" inert={mobile}><header className="topbar"><button className="icon-button menu-button" aria-label="Abrir menú" aria-expanded={mobile} aria-controls="main-navigation" onClick={event => { const trigger = event.currentTarget; afterMenuIconAnimation(() => { menuTrigger.current = trigger; setMobile(true); }); }}><AnimatedAlignCenter/></button>{role !== 'ADMIN' && <div className="property-select"><MapPin size={19}/><div><span>Tu vivienda</span><select aria-label="Seleccionar vivienda" value={property?.id ?? ''} onChange={e => { select(e.target.value); setMobile(false); }}>{properties.map(p => <option key={p.id} value={p.id}>{p.street} {p.houseNumber} · {p.cluster.name}</option>)}</select></div><ChevronDown size={15}/></div>}<div className="topbar-actions"><span className="online-label"><span className={`connection-dot ${online ? '' : 'offline'}`}/>{online ? 'En línea' : 'Sin conexión'}</span>{role !== 'ADMIN' && <button className="notification-button" aria-label={`Avisos de seguridad${alerts.data?.data.length ? ': ' + alerts.data.data.length : ''}`} onClick={() => go('/dispositivos')}><Bell size={20}/>{!!alerts.data?.data.length && <i/>}</button>}<span className="avatar top-avatar">{identity!.user.fullName.slice(0, 1)}</span></div></header><main id="main" tabIndex={-1} key={property?.id}>
    <div key={role === 'ADMIN' ? 'admin' : path} className={role === 'ADMIN' || path === '/' ? undefined : 'page-enter'}>
      {role === 'ADMIN' ? <Admin path={path} navigate={go}/> : role === 'GUARD' ? (path === '/caseta' ? <Gate/> : path === '/ayuda' ? <Help/> : path === '/dispositivos' && property ? <Devices/> : <Empty title="Esta página no está disponible" text="Vuelve a caseta para continuar." action={<Button onClick={() => go('/caseta')}>Ir a caseta</Button>}/>) : !property ? <Empty title="No hay viviendas disponibles" text="Contacta a administración para revisar tus membresías."/> : path === '/' || path === '/pases' ? <Passes key={path} home={path === '/'} navigate={go}/> : path === '/dispositivos' ? <Devices/> : path === '/ayuda' ? <Help/> : <Empty title="Esta página no está disponible" text="Vuelve al inicio para continuar." action={<Button onClick={() => go('/')}>Ir al inicio</Button>}/>}
    </div>
