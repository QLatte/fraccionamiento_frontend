...
function Shell({ path, navigate }: { path: string; navigate: (p: string) => void }) {
  ...
  function go(p: string) { setMobile(false); navigate(role === 'ADMIN' ? (adminSections.some(section => section.path === p) ? p : '/admin/invitaciones') : role === 'GUARD' && (p === '/' || p === '/pases' || p === '/dispositivos') ? '/caseta' : p); requestAnimationFrame(() => document.getElementById('main')?.focus()); }
