...
import { Bell, ChevronDown, CircleHelp, LogOut, MapPin, ScanLine, Settings2, ShieldCheck, Smartphone, WifiOff } from 'lucide-react';
...
function Shell({ path, navigate }: { path: string; navigate: (p: string) => void }) {
  ...
  const role = identity!.user.globalRole; const nav = [{ path: '/', label: 'Inicio', Icon: AnimatedHome }, { path: '/pases', label: 'Mis pases', Icon: AnimatedQr }, { path: '/dispositivos', label: 'Dispositivos', Icon: Smartphone }, ...(role !== 'HOUSEHOLD_USER' ? [{ path: '/caseta', label: 'Caseta', Icon: ScanLine }] : []), ...(role === 'ADMIN' ? [{ path: '/admin', label: 'Administración', Icon: Settings2 }] : [])];
