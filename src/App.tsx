...
function Router() {
  ...
  const admin = profile === 'ADMIN' || profile === 'SUPERADMIN';
...
function Shell({ path, navigate }: { path: string; navigate: (p: string) => void }) {
  const { identity, properties, property, select, logout, switchProfile } = useAuth(); const [mobile, setMobile] = useState(false); const online = useOnline();
  ...
  const role = identity!.session.profile;
  const alerts = useQuery<Page<Notification>>((role !== 'ADMIN' && role !== 'SUPERADMIN') && property ? `/auth/devices/notifications?propertyId=${property.id}` : null, 30000);
  ...
  function go(p: string) { setMobile(false); navigate((role === 'ADMIN' || role === 'SUPERADMIN') ? (adminSections.some(section => section.path === p) ? p : '/admin/invitaciones') : p); requestAnimationFrame(() => document.getElementById('main')?.focus()); }
  return <div className={`app-layout${(role === 'ADMIN' || role === 'SUPERADMIN') ? ' admin-layout' : ''}`}><a href="#main" className="skip-link">Ir al contenido</a>{mobile && <button className="sidebar-scrim" aria-label="Cerrar menú" onClick={() => setMobile(false)}/>}<aside ref={sidebar} id="main-navigation" role={mobile ? 'dialog' : undefined} aria-modal={mobile || undefined} aria-label="Menú de Zentry" className={`sidebar ${mobile ? 'open' : ''}`}><div className="sidebar-brand"><Brand onHome={() => go('/')}/><AnimatedX className="mobile-close" label="Cerrar menú" onClose={() => setMobile(false)}/></div><div className="community-label"><span className="connection-dot"/> Tu comunidad</div><nav aria-label="Navegación principal">{nav.map(({ path: p, label, Icon }) => <button key={p} className={path === p || ((role === 'ADMIN' || role === 'SUPERADMIN') && path.startsWith('/admin/')) ? 'active' : ''} aria-current={path === p || ((role === 'ADMIN' || role === 'SUPERADMIN') && path.startsWith('/admin/')) ? 'page' : undefined} onClick={() => { if (p === '/admin/invitaciones' && mobile) afterSlidersAnimation(() => go(p)); else go(p); }}><Icon size={20}/>{label}{(path === p || ((role === 'ADMIN' || role === 'SUPERADMIN') && path.startsWith('/admin/'))) && <span className="nav-dot"/>}</button>)}</nav>{(role !== 'ADMIN' && role !== 'SUPERADMIN') && <div className="sidebar-install"><div className="install-art"><Smartphone size={29}/><span><ShieldCheck size={15}/></span></div><h3>Zentry en tu teléfono</h3><p>Instala Zentry en tu teléfono y abre la app con un toque.</p><InstallButton/></div>}<div className="sidebar-bottom">{identity?.contexts?.admin?.length && role === 'RESIDENT' && <button onClick={() => void switchProfile('ADMIN', identity.contexts!.admin[0].clusterId)}>Cambiar a administración</button>}{identity?.contexts?.resident?.length && (role === 'ADMIN' || role === 'SUPERADMIN') && <button onClick={() => void switchProfile('RESIDENT')}>Cambiar a residente</button>}{(role !== 'ADMIN' && role !== 'SUPERADMIN') && <button onClick={() => go('/ayuda')} className={path === '/ayuda' ? 'active' : ''}><CircleHelp size={19}/> Ayuda y primeros pasos</button>}<button onClick={() => void logout().catch(() => {})}><LogOut size={19}/> Cerrar sesión</button><div className="sidebar-user"><span className="avatar">{identity!.user.fullName.slice(0, 1)}</span><span><strong>{identity!.user.fullName}</strong><small>{(role === 'ADMIN' || role === 'SUPERADMIN') ? 'Administración' : 'Residente'}</small></span></div></div></aside>
