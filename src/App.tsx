...
function Shell({ path, navigate }: { path: string; navigate: (p: string) => void }) {
  ...
  async function changeProfile(profile: 'RESIDENT' | 'ADMIN' | 'SUPERADMIN', clusterId?: string) { try { setProfileError(''); await switchProfile(profile, clusterId); } catch (e) { setProfileError(errorText(e)); } }
