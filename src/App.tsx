...
function Router() {
  ...
  useEffect(() => {
    const destination = path === '/caseta' || path.startsWith('/p/') ? null : admin ? ((profile === 'SUPERADMIN' && path === '/plataforma' || adminSections.some(section => section.path === path)) ? null : profile === 'SUPERADMIN' ? '/plataforma' : '/admin/invitaciones') : guard ? '/caseta' : identity && (path.startsWith('/admin') || path === '/plataforma') ? '/' : null;
...
function Shell({ path, navigate }: { path: string; navigate: (p: string) => void }) {
  ...
  // Profiles switch within the home used to log in; administering another community needs a login there.
  const sessionCluster = properties.find(p => p.id === identity!.session.propertyId)?.cluster.id;
  // Switching keeps the current session's expiry, so warn when little time is left.
  async function changeProfile(profile: 'RESIDENT' | 'ADMIN' | 'SUPERADMIN', clusterId?: string) {
    try {
      setProfileError(''); setProfileNotice('');
      const session = await switchProfile(profile, clusterId);
      const minutes = Math.ceil((new Date(session.expiresAt).getTime() - Date.now()) / 60_000);
      if (minutes <= 3) setProfileNotice(`Tu sesión vence en ${minutes} ${minutes === 1 ? 'minuto' : 'minutos'}. Después tendrás que ingresar de nuevo con tu llave de acceso.`);
  ...
  const nav = (role === 'ADMIN' || role === 'SUPERADMIN')
    ? [...(role === 'SUPERADMIN' ? [{ path: '/plataforma', label: 'Plataforma', Icon: ShieldCheck }] : []), { path: '/admin/invitaciones', label: 'Administración', Icon: AnimatedSlidersHorizontal }]
  ...
  function go(p: string) { setMobile(false); navigate((role === 'ADMIN' || role === 'SUPERADMIN') ? ((role === 'SUPERADMIN' && p === '/plataforma' || adminSections.some(section => section.path === p)) ? p : '/admin/invitaciones') : p); requestAnimationFrame(() => document.getElementById('main')?.focus()); }
