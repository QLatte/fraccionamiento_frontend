...
function Router() {
  ...
  useEffect(() => {
    const destination = admin && !path.startsWith('/p/') ? '/admin' : identity && path === '/admin' ? '/' : null;
    if (destination && path !== destination) {
      history.replaceState(null, '', destination);
      window.dispatchEvent(new PopStateEvent('popstate'));
    }
  }, [admin, identity, path]);
  return <><UpdateNotice/>{!online && <div className="offline-bar" role="status"><WifiOff size={17}/> Sin conexión. Puedes abrir la app; las acciones estarán disponibles al reconectarte.</div>}{path.startsWith('/p/') ? <PublicPass/> : !identity ? <Login/> : <Shell path={admin ? '/admin' : path} navigate={navigate}/>}</>;
}
function Shell({ path, navigate }: { path: string; navigate: (p: string) => void }) {
  ...
  const nav = role === 'ADMIN'
    ? [{ path: '/admin', label: 'Administración', Icon: AnimatedSlidersHorizontal }]
