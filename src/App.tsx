...
import { Bell, ChevronDown, CircleHelp, Home, LogOut, MapPin, Menu, QrCode, ScanLine, Settings2, ShieldCheck, Smartphone, WifiOff, X } from 'lucide-react';
...
function Shell({ path, navigate }: { path: string; navigate: (p: string) => void }) {
  ...
  const role = identity!.user.globalRole; const nav = [{ path: '/', label: 'Inicio', Icon: Home }, { path: '/pases', label: 'Mis pases', Icon: QrCode }, { path: '/dispositivos', label: 'Dispositivos', Icon: Smartphone }, ...(role !== 'HOUSEHOLD_USER' ? [{ path: '/caseta', label: 'Caseta', Icon: ScanLine }] : []), ...(role === 'ADMIN' ? [{ path: '/admin', label: 'Administración', Icon: Settings2 }] : [])];
  function go(p: string) { setMobile(false); navigate(p); requestAnimationFrame(() => document.getElementById('main')?.focus()); }
  return <div className="app-layout"><a href="#main" className="skip-link">Ir al contenido</a>{mobile && <button className="sidebar-scrim" aria-label="Cerrar menú" onClick={() => setMobile(false)}/>}<aside ref={sidebar} id="main-navigation" role={mobile ? 'dialog' : undefined} aria-modal={mobile || undefined} aria-label="Menú de SICA" className={`sidebar ${mobile ? 'open' : ''}`}><div className="sidebar-brand"><Brand onHome={() => go('/')}/><button className="icon-button mobile-close" aria-label="Cerrar menú" onClick={() => setMobile(false)}><X/></button></div><div className="community-label"><span className="connection-dot"/> Tu comunidad</div><nav aria-label="Navegación principal">{nav.map(({ path: p, label, Icon }) => <button key={p} className={path === p ? 'active' : ''} aria-current={path === p ? 'page' : undefined} onClick={() => go(p)}><Icon size={20}/>{label}{path === p && <span className="nav-dot"/>}</button>)}</nav><div className="sidebar-install"><div className="install-art"><Smartphone size={29}/><span><ShieldCheck size={15}/></span></div><h3>SICA en tu teléfono</h3><p>Instala SICA en tu teléfono y abre la app con un toque.</p><InstallButton/></div><div className="sidebar-bottom"><button onClick={() => go('/ayuda')} className={path === '/ayuda' ? 'active' : ''}><CircleHelp size={19}/> Ayuda y primeros pasos</button><button onClick={() => void logout().catch(() => {})}><LogOut size={19}/> Cerrar sesión</button><div className="sidebar-user"><span className="avatar">{identity!.user.fullName.slice(0, 1)}</span><span><strong>{identity!.user.fullName}</strong><small>{role === 'ADMIN' ? 'Administración' : role === 'GUARD' ? 'Personal de caseta' : 'Residente'}</small></span></div></div></aside>
